...
int main(int argc, char *argv[]) {
    ...
    // Gather results from all processes (you may need to adjust this part)
    ...
    if (rank == 0) {
        saveBitmap("mandelbrotmpidynamic.bmp", globalImageData, WIDTH, HEIGHT);
        free(globalImageData);

        // Print the execution time
        printf("Execution Time: %lf seconds\n", execution_time);

        // Write the execution time to an external file
        FILE *time_file = fopen("execution_time_mpi_dynamic.txt", "w");
        if (time_file == NULL) {
...
mpicc -o mandelbrot_mpi_dynamic mandelbrot_mpi_dynamic.c
mpirun -n 4 ./mandelbrot_mpi_dynamic
